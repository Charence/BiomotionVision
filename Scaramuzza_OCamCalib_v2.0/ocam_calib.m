...
cell_list{1,1} = {'Read names','data_calib;'};
cell_list{1,2} = {'Extract grid corners','click_calib;'};
cell_list{1,3} = {'Calibration','calibration;'};
cell_list{2,1} = {'Find center','findcenter;'};
cell_list{2,2} = {'Calibration Refinement','optimizefunction;'};
cell_list{2,3} = {'Reproject on images','reproject_calib;'};
cell_list{3,1} = {'Show Extrinsic','create_simulation_points;'};
cell_list{3,2} = {'Analyse error','analyse_error;'};
cell_list{3,3} = {'Recomp. corners','recomp_corner_calib;'};
cell_list{4,1} = {'Show calib results','show_calib_results;'};
cell_list{4,2} = {'Save','saving_calib;'};
cell_list{4,3} = {'Load','loading_calib;'};
cell_list{5,1} = {'Export Data','exportData2TXT;'};
cell_list{5,2} = {'Exit',['disp(''Bye. To run again, type ocam_calib.''); close(' num2str(fig_number) ');']}; %{'Exit','calib_gui;'};
