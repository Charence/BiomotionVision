...
while (i <= n_ima), % & (~no_image_file),
   ...
   if active_images(i),
      ...
      if exist(ima_name),
        ...
        if ~strcmp('jpg',format_image) %If image format is not JPG then converts to JPG! Needed only for OpenCV functions!
            imwrite(uint8(Ii), [calib_name  number_ext '.' 'jpg'], 'jpg','Quality',100); %converts to JPG
            l(i).name = [calib_name  number_ext '.' 'jpg'];
        end
